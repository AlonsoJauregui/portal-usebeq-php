<?php
	...
	$tipo_tramite = "CERTIFICADO DE SECUNDARIA";
	...
	$grado = "";
	$reprobo_materias = $_POST['reprobo_materias'];

	if (isset($_POST['presento_extras'])) {
		$presento_extras_en = $_POST['presento_extras'];
	}else{
		$presento_extras_en = "";
	}


	$year_extras = $_POST['year_extras'];
	$estudio_siempre = $_POST['estudio_siempre'];
	$nom_esc_ant = $_POST['nom_esc_ant'];
	...
	if ( $longitud_curp == '18' ) {
		...
		/*$verifica_certificado = $conexion->query("SELECT COUNT(*) AS CER FROM SCE039 WHERE al_curp = '$curp' AND nivel = 'SEC'");
		...
		//VALIDACIÓN PARA SABER SI EL TRÁMITE ES DEL ESTADO Y DEL NIVEL CORRESPONDIENTE
		if ($estado_cct == 22 AND ($nivel_cct == 'DST' OR $nivel_cct == 'DES' OR $nivel_cct == 'DTV' OR $nivel_cct == 'PES' OR $nivel_cct == 'DML' OR $nivel_cct == 'DBA' OR $nivel_cct == 'ADG' OR $nivel_cct == 'EES' OR $nivel_cct == 'DSN')) {
			...
			if ($nivel_busca == 'SEC') {
				...
				if ($check == "SI") {
					...
					switch ($caso) {
						case 1:
							...
							$responsable = $conexion -> query("SELECT * FROM RESPONSABLES WHERE CLAVE = '$cct' AND NIVEL = 'SEC'");
							...
							if ($respuesta !== false) {
								...
								if ($ciclo_terminacion == "2000-2001" or $ciclo_terminacion == "2001-2002" or $ciclo_terminacion == "2002-2003" or $ciclo_terminacion == "2003-2004" or $ciclo_terminacion == "2004-2005" or $ciclo_terminacion == "2005-2006" or $ciclo_terminacion == "2006-2007" or $ciclo_terminacion == "2007-2008" or $ciclo_terminacion == "2008-2009" or $ciclo_terminacion == "2009-2010" or $ciclo_terminacion == "2010-2011" or $ciclo_terminacion == "2011-2012" or $ciclo_terminacion == "2012-2013" or $ciclo_terminacion == "2013-2014" or $ciclo_terminacion == "2014-2015" or $ciclo_terminacion == "2015-2016" or $ciclo_terminacion == "2016-2017" or $ciclo_terminacion == "2017-2018" or $ciclo_terminacion == "2018-2019" or $ciclo_terminacion == "2019-2020" or $ciclo_terminacion == "2020-2021" or $ciclo_terminacion == "2021-2022" or $ciclo_terminacion == "2022-2023" or $ciclo_terminacion == "2023-2024" or $ciclo_terminacion == "2024-2025") {
									...
									$statement = $conexion->prepare('INSERT INTO tramites1 (folio, nombre_alumno, a_paterno, a_materno, telefono, email, curp, cct, nombre_esc, dom_esc, turno, ciclo_terminacion, tipo_escuela, grado, reprobo_materias, presento_extras_en, year_extras, estudio_siempre, nom_esc_ant, sexo, tipo_tramite, usuario, foto, fecha, status, region, correccion, core) VALUES (:folio, :nombre_alumno, :a_paterno, :a_materno, :telefono, :email, :curp, :cct, :nombre_esc, :dom_esc, :turno, :ciclo_terminacion, :tipo_escuela, :grado, :reprobo_materias, :presento_extras_en, :year_extras, :estudio_siempre, :nom_esc_ant, :sexo, :tipo_tramite, :usuario, :foto, :fecha, :status, :region, :correccion, :core)');

									$statement->execute(array(":folio"=>$folio, ":nombre_alumno"=>$nombre_alumno, ":a_paterno"=>$a_paterno, ":a_materno"=>$a_materno, ":telefono"=>$tel, ":email"=>$email, ":curp"=>$curp, ":cct"=>$cct, ":nombre_esc"=>$nombre_esc, ":dom_esc"=>$dom_esc, ":turno"=>$turno, ":ciclo_terminacion"=>$ciclo_terminacion, ":tipo_escuela"=>$tipo_escuela, ":grado"=>$grado, ":reprobo_materias"=>$reprobo_materias, ":presento_extras_en"=>$presento_extras_en, ":year_extras"=>$year_extras, ":estudio_siempre"=>$estudio_siempre, ":nom_esc_ant"=>$nom_esc_ant, ":sexo"=>$grado_ant, ":tipo_tramite"=>$tipo_tramite, ":usuario"=>$usuario, ":foto"=>$foto, ":fecha"=>$fecha, ":status"=>$status, ":region"=>$region, ":correccion"=>$check, ":core"=>$core));
									...
									$statement = $conexion->prepare('INSERT INTO tramites1 (folio, nombre_alumno, a_paterno, a_materno, telefono, email, curp, cct, nombre_esc, dom_esc, turno, ciclo_terminacion, tipo_escuela, grado, reprobo_materias, presento_extras_en, year_extras, estudio_siempre, nom_esc_ant, sexo, tipo_tramite, foto, fecha, status, region, correccion, core) VALUES (:folio, :nombre_alumno, :a_paterno, :a_materno, :telefono, :email, :curp, :cct, :nombre_esc, :dom_esc, :turno, :ciclo_terminacion, :tipo_escuela, :grado, :reprobo_materias, :presento_extras_en, :year_extras, :estudio_siempre, :nom_esc_ant, :sexo, :tipo_tramite, :foto, :fecha, :status, :region, :correccion, :core)');

									$statement->execute(array(":folio"=>$folio, ":nombre_alumno"=>$nombre_alumno, ":a_paterno"=>$a_paterno, ":a_materno"=>$a_materno, ":telefono"=>$tel, ":email"=>$email, ":curp"=>$curp, ":cct"=>$cct, ":nombre_esc"=>$nombre_esc, ":dom_esc"=>$dom_esc, ":turno"=>$turno, ":ciclo_terminacion"=>$ciclo_terminacion, ":tipo_escuela"=>$tipo_escuela, ":grado"=>$grado, ":reprobo_materias"=>$reprobo_materias, ":presento_extras_en"=>$presento_extras_en, ":year_extras"=>$year_extras, ":estudio_siempre"=>$estudio_siempre, ":nom_esc_ant"=>$nom_esc_ant, ":sexo"=>$grado_ant, ":tipo_tramite"=>$tipo_tramite, ":foto"=>$foto, ":fecha"=>$fecha, ":status"=>'SOLICITADO SIN RESPONSABLE', ":region"=>$region, ":correccion"=>$check, ":core"=>$core));
									$resultado = $statement -> fetch();
				...
				if ($check == "NO") {
					...
					if ($cuenta != 0 AND $val_tram == 0) {
						...
						$statement = $conexion->prepare('INSERT INTO tramites1 (folio, nombre_alumno, a_paterno, a_materno, telefono, email, curp, cct, nombre_esc, dom_esc, turno, ciclo_terminacion, tipo_escuela, grado, reprobo_materias, presento_extras_en, year_extras, estudio_siempre, nom_esc_ant, sexo, tipo_tramite, usuario, foto, fecha, fecha_elaborado, status, region, correccion, core) VALUES (:folio, :nombre_alumno, :a_paterno, :a_materno, :telefono, :email, :curp, :cct, :nombre_esc, :dom_esc, :turno, :ciclo_terminacion, :tipo_escuela, :grado, :reprobo_materias, :presento_extras_en, :year_extras, :estudio_siempre, :nom_esc_ant, :sexo, :tipo_tramite, :usuario, :foto, :fecha, :fecha_elaborado, :status, :region, :correccion, :core)');

						$statement->execute(array(":folio"=>$folio, ":nombre_alumno"=>$nombre_alumno, ":a_paterno"=>$a_paterno, ":a_materno"=>$a_materno, ":telefono"=>$tel, ":email"=>$email, ":curp"=>$curp, ":cct"=>$cct, ":nombre_esc"=>$nombre_esc, ":dom_esc"=>$dom_esc, ":turno"=>$turno, ":ciclo_terminacion"=>$ciclo_terminacion, ":tipo_escuela"=>$tipo_escuela, ":grado"=>$grado, ":reprobo_materias"=>$reprobo_materias, ":presento_extras_en"=>$presento_extras_en, ":year_extras"=>$year_extras, ":estudio_siempre"=>$estudio_siempre, ":nom_esc_ant"=>$nom_esc_ant, ":sexo"=>$grado_ant, ":tipo_tramite"=>$tipo_tramite, ":usuario"=>'SISCER', ":foto"=>$foto, ":fecha"=>$fecha, ":fecha_elaborado"=>$fecha_elaborado, ":status"=>'REIMPRESION', ":region"=>$region, ":correccion"=>$check, ":core"=>$core));
						$resultado = $statement -> fetch();
                  ...
                  <div class="row">
					...
					<?php } if ($val_tram == 5) { ?>
                  		...
                  		<div class="row col-12 justify-content-center">
							<h4 align="center" class="">El alumno (a) egresó de SECUNDARIA en el ciclo escolar 2022 - 2023 por lo tanto para consulta y descarga de sus documentos de acreditación ingrese a: <a href="http://portal.usebeq.edu.mx">portal.usebeq.edu.mx</a></h4>
							<br>
						</div>
					...
					<?php } if ($val_tram == 6) { ?>
                  		...
                  		<div class="row col-12 justify-content-center">
							<h4 align="center" class="">El alumno (a) se encuentra inscrito en SECUNDARIA para consulta y descarga de boletas de evaluación ingrese a: <a href="http://portal.usebeq.edu.mx">portal.usebeq.edu.mx</a></h4>
							<br>
						</div>
					...
					<?php } if ($val_tram == 8) { ?>
						...
						<div class="row col-12 justify-content-center">
							<h4 align="center" class="">La clave de la escuela ingresada no corresponde a SECUNDARIA, favor de seleccionar el nivel correcto del trámite a realizar en el menú de solicitud de duplicados de certificado.</a></h4>
							<br>
						</div>

<?php session_start();
  ...
  $datos = $conexion->query("SELECT usuario, u_id, u_tel, u_nombre, u_appat, u_apmat, domicilio, sexo FROM PP_usuarios WHERE u_correo = '$correo'");
  ...
  $alum = $conexion->query("SELECT al_id, al_nombre, al_appat, al_apmat FROM pp_alumnos WHERE (padre = '$correo' OR madre = '$correo' OR tutor = '$correo')");
...
<head>
  ...
  <script async src="https://www.googletagmanager.com/gtag/js?id=UA-170519199-1"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());
    gtag('config', 'UA-170519199-1');
  </script>
        ...
        <div class="header-body">
          ...
          <div class="row">
            ...
            <?php while ($res = $alum -> fetch()) { 
              ...
              $niv_compara = $conexion->query("SELECT dbo.SCE006.eg_id, dbo.SCE002.eg_grado, dbo.SCE002.clavecct, dbo.SCE002.nivel FROM dbo.SCE002 INNER JOIN dbo.SCE006 ON dbo.SCE002.eg_id = dbo.SCE006.eg_id WHERE (dbo.SCE006.al_id = '$al_id_compara')"); 

              foreach ($niv_compara as $key) {
                 $nivel_al = $key['nivel'];
                 $clavecct = $key['clavecct'];
                 $eg_grado = $key['eg_grado'];
              } 

              $rest = substr($clavecct, 2, 3);

              if ($rest == 'PJN' OR $rest == 'PPR' OR $rest == 'PES') {
              	$caso = 'SI';
              }
              else {
              	$caso = 'NO';
              ...
              $certi = $conexion->query("SELECT IdEstatus FROM SCE039 WHERE al_id = '$al_id_compara'");
              ...
              $al_est = $conexion->query("SELECT al_estatus, al_fecing FROM SCE004 WHERE al_id = '$al_id_compara'");

              foreach ($al_est as $clave) {
                 $al_estatus = rtrim($clave['al_estatus']);
                 $al_fecing = $clave['al_fecing'];
              } 

              $al_fecing = strtotime($al_fecing);
              $year_ing = date('Y', $al_fecing);
            ...
            <div class="modal fade" id="<?php echo "modal".$res['al_id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <h5 class="card-title text-uppercase text-muted mb-0"><?php echo $res['al_nombre']." ".$res['al_appat']." ".$res['al_apmat'] ?></h5>
                    <?php if ($al_estatus == "I" OR $al_estatus == "A" OR $al_estatus == "R") { ?>
                    <!--<a href="calificaciones.php?al_id=<?php echo $res['al_id'] ?>"><i>• Calificaciones</i></a><br>-->
                    <?php if ($caso == 'NO') { ?>
                      <!--<a href="materias.php?al_id=<?php echo base64_encode($res['al_id']) ?>"><i>• Mis Maestros ciclo 2020-2021</i></a><br>-->
                	  <?php } ?>
                    <a href="ReporteE/api_boletas/boleta_ev2024.php?al_id=<?php echo base64_encode($res['al_id']) ?>" target="_blank"><i>• Boleta de Evaluación 2023-2024</i></a><br>
                    <a href="ReporteE/api_boletas/boleta_ev2025.php?al_id=<?php echo base64_encode($res['al_id']) ?>" target="_blank"><i>• Boleta de Evaluación 2024-2025</i></a><br>
                    <a href="ReporteE/ReporteCC_2122.php?al_id=<?php echo base64_encode($res['al_id']) ?>" target="_blank"><i>• Reporte Componentes Curriculares 2021-2022</i></a><br>
                    <a href="ReporteE/ReporteCC_2223.php?al_id=<?php echo base64_encode($res['al_id']) ?>" target="_blank"><i>• Reporte Componentes Curriculares 2022-2023</i></a><br>
                    <?php if ($cr == 4) { ?>
                      <a href="https://portal.usebeq.edu.mx/certificados2/2425/<?php echo $res['al_id'] ?>.pdf" target="_blank"><i>• Certificado Electrónico ciclo 2024-2025</i></a><br>
                    <?php } ?>
                    <?php if ($cr == 'Promedio') { ?>
                      <i>• Para la emisión del documento de acreditación es necesario completar el historial académico en la escuela correspondiente, contacte al Directivo Escolar.</i><br>
                    <?php } ?>
            ...
            <div class="col-xl-3 col-lg-6 pb-4">
              <div class="card card-stats mb-4 mb-xl-0">
                <div class="card-body">
                  <div class="row">
                    <div class="col">
                      <h4 class="card-title text-uppercase text-muted mb-0"><b>Agregar Estudiante</b></h4>
                    </div>
                    <div class="col-auto">
